...
class StableDiffusionRelightPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                results = self.scheduler.step(noise_pred, t, latents[:, :4], **extra_step_kwargs, return_dict=True)
                latents = torch.concat([results.prev_sample, latents[:, 4:]], dim=1)
        ...
        if not output_type == "latent":
            image = self.vae.decode(latents[:,:4] / self.vae.config.scaling_factor, return_dict=False)[0]
