...
from hydit.data_loader.arrow_load_stream import TextImageArrowStream, RelightingArrowStream
...
def prepare_model_inputs(args, batch, device, vae, text_encoder, text_encoder_t5, freqs_cis_img):
    ...
    _, _, height, width = image[0].shape
...
def main(args):
    ...
    # @modify: from TextImageArrowStream to RelightingArrowStream
    dataset = RelightingArrowStream(args=args,
    ...
    with torch.no_grad():
        if args.use_fp16:
            ...
            # if args.relight_mode == "fg":
            #     new_embedder_proj.weight[:, 4:8, :, :].copy_(model.module.x_embedder.proj.weight)
            # elif args.relight_mode == "bg":
            #     new_embedder_proj.weight[:, 4:8, :, :].copy_(model.module.x_embedder.proj.weight)
            #     new_embedder_proj.weight[:, 8:, :, :].copy_(model.module.x_embedder.proj.weight)
            ...
            # Load weight from hydit model
            # if args.relight_mode == "fg":
            #     new_embedder_proj.weight[:, 4:8, :, :].copy_(model.x_embedder.proj.weight)
            # elif args.relight_mode == "bg":
            #     new_embedder_proj.weight[:, 4:8, :, :].copy_(model.x_embedder.proj.weight)
            #     new_embedder_proj.weight[:, 8:, :, :].copy_(model.x_embedder.proj.weight)
