...
from .diffusion.pipeline_relight import StableDiffusionRelightPipeline as StableDiffusionPipeline
...
class End2End(object):
    def __init__(self, args, models_root_path):
        ...
        self.vae = AutoencoderKL.from_pretrained(str(vae_path)).to(self.device).half()
        ...
        if self.infer_mode in ['fa', 'torch']:
            ...
            if args.relight_mode == "fg":
                new_embedder_proj = torch.nn.Conv2d(8, self.model.x_embedder.proj.out_channels, self.model.x_embedder.proj.kernel_size, self.model.x_embedder.proj.stride)
            elif args.relight_mode == "bg":
                new_embedder_proj = torch.nn.Conv2d(12, self.model.x_embedder.proj.out_channels, self.model.x_embedder.proj.kernel_size, self.model.x_embedder.proj.stride)
            else:
                raise NotImplementedError("reilght_mode {} is not implemented.".format(args.relight_mode))
            self.model.x_embedder.proj = new_embedder_proj
            # proj.bias = self.model.x_embedder.proj.bias.to(torch.float16) 
            self.load_torch_weights()
            self.model.x_embedder = self.model.x_embedder.to(torch.float16)
    ...
    def predict(self,
        ...
        if src_size_cond is None:
         ...
         # ========================================================================
        # Arguments: image. (input fg and bg image, required for relighting)
        # ========================================================================
        if image is None:
            raise ValueError(f" image is None, must have a input image to relight.")
